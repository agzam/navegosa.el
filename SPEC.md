...
- [done] `mediaFullscreenToggle(browser, w, t)` - the player's own fullscreen. JS from Apple Events carries no user activation, so a synthetic click alone makes the Fullscreen API fire `fullscreenerror`; a key posted to the browser process with `CGEventPostToPid` (F19, bound by nothing) is trusted input to the window's front tab while the browser stays in the background, and gives the page transient activation. Then `_mediaJS` `fullscreenToggle` clicks `.ytp-fullscreen-button` (else `video.requestFullscreen()`), and the call waits up to 1s for `document.fullscreenElement` to confirm. Exiting is `document.exitFullscreen()`, gesture-free. Needs Accessibility permission for osascript; without it the page reports no activation and the command says so. Replaces the V3 macOS window fullscreen (System Events `AXFullScreen`), which maximized the browser window instead of the video (E2E-verified)
- [done] Focus contract: media commands never activate the browser. `showTab(browser, w, t)` switches the tab in its window and raises the window within the browser only; `activateTab` is `showTab` plus app activation and stays with the general tab switchers. Entering player fullscreen activates the browser twice - on the request and when the animation ends ~0.7s later - so `navegosa-media-fullscreen-toggle` first starts `navegosa--reclaim-focus`, a fire-and-forget JS `reclaimFocus(browser, pid, waitMs)` that re-activates Emacs every time the browser is frontmost until its deadline (E2E through the command: browser in front 0.3s after the key press, Emacs back 89ms later; exiting takes no focus)
...
- DOM commands (play/pause, theater, status) never move keyboard focus, fullscreen browser window or not; the focus thefts came from `activateTab` (select-tab, open-tab) and from entering fullscreen.
- `CGEventPostToPid` keys reach the front tab of a background Chromium browser as trusted input (`isTrusted`, `navigator.userActivation.isActive` true about 12ms later) without activating it.
- Player fullscreen timings (Brave, macOS 26.7): `document.fullscreenElement` set ~110ms after the button click, cleared ~100ms after `exitFullscreen()`. The request activates the browser at once, the end of the animation activates it again ~0.7s later. A watcher that stopped after its first reclaim let keys typed in between reach YouTube - its `i` opens the miniplayer, which leaves fullscreen and navigates the tab home.
...
- 141 buttercup specs pass (unit + mocked integration + real-subprocess timeout guard + mocked D-Bus + mocked hyprctl/pactl)
...
- 2026-09-25 (later): player fullscreen replaces window fullscreen. A `CGEventPostToPid` key gives the page the user activation the Fullscreen API needs without activating the browser; YouTube's button does the rest, `exitFullscreen()` undoes it. `reclaimFocus` now reclaims until its deadline, since entering activates the browser twice. `windowFullscreenToggle` removed. 141 specs.
